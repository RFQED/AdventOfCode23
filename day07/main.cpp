...
std::tuple<int, int> classify_string_part2(const string& s);
bool comparePairs(const pair<string, int>& a, const pair<string, int>& b);
bool comparePairs_part2(const pair<string, int>& a, const pair<string, int>& b);
...
int part2(){
    ...
    for (auto & hand : hands) {
        auto [max_count, second_max_count] = classify_string_part2(hand.first);
        // Classify based on frequencies
        if (max_count > 4) five_kind.push_back(hand);
        else if (max_count == 4) four_kind.push_back(hand);
        else if (max_count == 3 && second_max_count == 2) full_house.push_back(hand);
        else if (max_count == 3) three_kind.push_back(hand);
        else if (max_count == 2 && second_max_count == 2) two_pair.push_back(hand);
    ...
    // Sort the vector using the custom comparison function
    sort(five_kind.begin(), five_kind.end(), comparePairs_part2);
    sort(four_kind.begin(), four_kind.end(), comparePairs_part2);
    sort(full_house.begin(), full_house.end(), comparePairs_part2);
    sort(three_kind.begin(), three_kind.end(), comparePairs_part2);
    sort(two_pair.begin(), two_pair.end(), comparePairs_part2);
    sort(one_pair.begin(), one_pair.end(), comparePairs_part2);
    sort(high_card.begin(), high_card.end(), comparePairs_part2);
